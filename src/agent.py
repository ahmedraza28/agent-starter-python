...
async def _memory_heartbeat(room_name: str) -> None:
    while True:
        await asyncio.sleep(MEMORY_LOG_INTERVAL_SEC)
        log_memory_snapshot("heartbeat", room=room_name)
...
async def my_agent(ctx: JobContext):
    ...
    if MEMORY_LOG_INTERVAL_SEC > 0:
        heartbeat_task = asyncio.create_task(
            _memory_heartbeat(ctx.room.name),
    ...
    try:
        ...
        log_memory_snapshot("session_started", room=ctx.room.name)

        # Join the room and connect to the user
        await ctx.connect()
        log_memory_snapshot("room_connected", room=ctx.room.name)
        await session.generate_reply(
            instructions="Greet the user and offer your assistance."
        )
        log_memory_snapshot("initial_reply_generated", room=ctx.room.name)
    except Exception:
        log_memory_snapshot("job_error", room=ctx.room.name)
        if heartbeat_task is not None and not heartbeat_task.done():
            heartbeat_task.cancel()
        raise
